...
#             raise Exception(f'{self.time_space}  must be a multiple of {self.prior_hours}.')
    ...
    def build(self, input_shape):
        ...
        self.w_t = self.add_weight(shape=(self.no_reference_time_point,self.axis_2,input_shape[0][2],4),
                                 initializer='glorot_uniform',
                                 name='w_tempo',
                                 dtype='float32',
                                 trainable=True)
    ...
    def call(self, inputs,training=None):
        ...
        if training:
            ...
            self.lattent_x=self.dropout_1(tf.nn.relu(tf.reduce_sum(self.w_t*tf.concat([self.x_s,self.intensity_s,self.m_s,self.dt],4)+self.b_t,4)),training=training)
            #Feature-level aggregation
            self.lattent_x=self.dropout_2(tf.nn.relu(tf.reduce_sum(self.w_v*self.lattent_x + self.b_v,2)),training=training)
        else:
            #Value-level extraction
            self.lattent_x=tf.nn.relu(tf.reduce_sum(self.w_t*tf.concat([self.x_s,self.intensity_s,self.m_s,self.dt],4)+self.b_t,4))
            #Feature-level aggregation
...
class ALNN(keras.Model):
    ...
    def call(self, inputs,training=None):
        ...
        self.pseudo_aligned=self.alnnlayer([self.values,self.timestamps,self.masks,self.timevariations])
...
#                  gru_imputer_unit,
#                  nr_variable,
#                  dropout_gru_imputer_unit=0.0,
#                  time_controleur=False,
#                  with_lann=True,
#                  nr_time_point=1,
#                  lann_gru_unit=168,
...
#         self.nr_variable=nr_variable
#         self.dropout_gru_imputer_unit=dropout_gru_imputer_unit
#         self.imputation=imputation
#         self.time_controleur=time_controleur
#         self.with_lann=with_lann
#         self.nr_time_point=nr_time_point
#         self.lann_gru_unit=lann_gru_unit
        ...
        if task_type=="regression":
            ...
            self.classifier=layers.Dense(1,name="classifier")
...
#         self.padd_brits=layers.RNN(self.padd_brits_layers,return_sequences=True)
#         self.gru=layers.GRU(168,dropout=0.0,return_sequences=False)
#         #self.time_distributed=layers.TimeDistributed(layers.Dense(self.nr_variable))
#         self.classifier=layers.Dense(1,name="classifier")
        self.mae = tf.keras.losses.MeanAbsoluteError()
#         self.mse = tf.keras.losses.MeanSquaredError()

#         self.mdl=LANN_GRU(48,init_time=0,nr_time_point=self.nr_time_point,gru_unit=self.lann_gru_unit,type_distance="abs")
#         self.gru_2=layers.GRU(20,return_sequences=False)
...
#         self.dropout=layers.Dropout(0.1)
    ...
    def call(self, inputs,training=None):
        ...
        #Data-driven imputation with paddgru
        if self.is_imputation:
            ...
            if self.is_alignment:
                feaures_embedded_=self.alnn([feaures_embedded_,time_stamps,masks,delta])
            else:
                feaures_embedded_=self.gru(feaures_embedded_)
        else:
            feaures_embedded_=self.alnn([values,time_stamps,masks,delta])
            

        return self.classifier(feaures_embedded_)
